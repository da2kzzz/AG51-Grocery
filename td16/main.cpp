...
protected:
    ...
    void InOrder(BST *r,void (*Visit)(const int &)) const;
    ...
    void findKey(int k, BST *r,bool &e);
public:
    ...
    BST *find(int k);
    bool insert(int k);
    template<size_t N> int insert(int (&tabKey)[N]);
    bool findKey(int k);
};
...
void BinaryTree::findKey(int k, BST *r,bool &e){
    if (r != nullptr ) {
        if(r->data==k) {
            e= true;
        }
        else if(k<r->data){
            if(r->leftChild!= nullptr) findKey(k,r->leftChild,e);
            else e= false;
        }
        else if(k>r->data){
            if(r->rightChild!= nullptr) findKey(k,r->rightChild,e);
            else e= false;
        }
    }
    else return;
...
bool BinaryTree::findKey(int k) {
    bool e= false;
    findKey(k,root,e);
    return e;
}

template <size_t N> int  BinaryTree::insert(int (&tabKey)[N]){
    for(int i=0;i<N;i++){
        insert((tabKey)[i]);
    }
    return 1;
...
int main() {
    ...
    cout<<bt.findKey(45)<<endl;//true==1;false==0
    cout<<bt.insert(11)<<endl;//true==1;false==0
    //bt.InOrder(Write);
    int arr[11]={16,8,11,7,9,21,30,25,30,46,14};
    bt.insert(arr);
    bt.InOrder(Write);
}

...
private:
    ...
    Icon *park;
    vector<CarData*> Cars;
    vector<TextureData*> servers;
    vector<TextureData*> parks;
    vector<TextureData*> pass;
    vector<TextureData*>::iterator mouseOverServer;
    TextureData* selectedServer;
    string configFile;
    double fieldSurface;
public:
...
void PolygonDraw::onStart() {
    ...
    park=new Icon(48.0,64.0,"../textures/carpark.tga");
    car = new Icon(64,64,"../textures/bluecar.tga");
    loadServerPositions(configFile);
    int lx,ly;
    parkId = loadTGATexture("../textures/carpark.tga",lx,ly);
    assert(parkId!=0);
    passId = loadTGATexture("../textures/carstop.tga",lx,ly);
    assert(passId!=0);
    ...
    //int tabTri[][3]={{0,4,5},{0,5,1},{1,5,3},{1,3,2},{3,5,6},{3,6,2},{5,2,6},{5,7,2},{5,8,7},
                    // {5,9,8},{7,8,2}};
...
void PolygonDraw::onMouseDown(int button, double cx, double cy) {
    ...
    Triangle *thistri = mesh->getTriangle(v);
    Triangle *neigbr = mesh->neighborInside(thistri);
    if(neigbr!= nullptr){
        ...
        if (fin.is_open()) {
            ...
            while (!fin.eof()) {
                ...
                if (strLine.size()>1) {
                    ...
                    servers.push_back(new TextureData(name, position, color));
                }
            }
        }
        fin.close();
